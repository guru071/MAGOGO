...
import { isNative, nativeGoogleLogin } from '@/lib/native-bridge'
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (isNative) {
        ...
        if (result?.email) {
          await useStore.getState().fetchMe()
    ...
    <Dialog open={showAuthModal} onOpenChange={setShowAuthModal}>
      <DialogContent className="sm:max-w-md bg-card border-border p-8 rounded-sm">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-card hover:bg-muted text-foreground font-medium h-12 rounded-sm border border-border transition-all disabled:opacity-50"
        >
          {googleLoading ? <><Loader2 className="h-5 w-5 animate-spin" /> Continue with Google</> : <><svg className="h-5 w-5" viewBox="0 0 24 24">
